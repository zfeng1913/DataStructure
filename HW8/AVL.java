...
public class AVL<T extends Comparable<? super T>> {
    ...
    public void updateHeightAndBF(AVLNode<T> currentNode) {
        ...
        if (currentNode.getLeft() == null){
            ...
            left_height = currentNode.getLeft().getHeight();
        }
        if (currentNode.getRight() == null){
            right_height = -1;
        }else{
            right_height = currentNode.getRight().getHeight();
        }
        currentNode.setHeight(Math.max(right_height, left_height) + 1);
        currentNode.setBalanceFactor(left_height - right_height);
